...
const PromptForm: React.FC<PromptFormProps> = ({
  ...
  const [videoStyle, setVideoStyle] = useState<VideoStyle>(
    initialValues?.videoStyle ?? VideoStyle.NONE,
  );
  ...
  useEffect(() => {
    if (initialValues) {
      ...
      setVideoStyle(initialValues.videoStyle ?? VideoStyle.NONE);
  ...
  const handleSubmit = useCallback(
    (e: React.FormEvent) => {
      e.preventDefault();
      onGenerate({
        prompt: `${prompt.trim()}${videoStylePrompts[videoStyle]}`,
        model,
        aspectRatio,
        resolution,
        mode: generationMode,
  ...
  switch (generationMode) {
    case GenerationMode.TEXT_TO_VIDEO:
      isSubmitDisabled = !prompt.trim() && videoStyle === VideoStyle.NONE;
      if (isSubmitDisabled) {
        tooltipText = 'Please enter a prompt or select a style.';
      }
      break;
