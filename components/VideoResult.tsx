...
import {CssFilter, cssFilterValues} from '../types';
...
const VideoResult: React.FC<VideoResultProps> = ({
  ...
  const [activeFilter, setActiveFilter] = useState<CssFilter>(CssFilter.NONE);
    ...
    <div className="w-full flex flex-col items-center gap-8 p-8 bg-gray-800/50 rounded-lg border border-gray-700 shadow-2xl">
      ...
      <div className="w-full max-w-2xl aspect-video rounded-lg overflow-hidden bg-black shadow-lg">
        <video
          ...
          style={{filter: cssFilterValues[activeFilter]}}
        />
      </div>

      <div className="w-full max-w-2xl">
        <label className="text-sm block mb-2 font-medium text-gray-400 text-center">
          Color Filter
        </label>
        <div className="flex flex-wrap gap-2 justify-center">
          {Object.values(CssFilter).map((filter) => (
            <button
              key={filter}
              type="button"
